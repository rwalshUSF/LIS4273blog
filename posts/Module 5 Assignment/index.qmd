...
round(cor(data.set$Girls.Time,data.set$Girls.Grades,method="pearson"),digits=2)
# This result is close to 1 so a strong positive relation is implied.

# The Pearson correlation coefficient between time spent and grade
# for the Boys' dataset:
round(cor(data.set$Boys.Time,data.set$Boys.Grades,method="pearson"),digits=2)
# This result is close to 1 so a strong positive relation is implied.

# This is the multivariable matrix's correlation analysis
# including all variables, using two different tests:

# As pearson correlation
round(cor(data.set,method="pearson"),digits=2)

#As spearman correlation
round(cor(data.set, method="spearman"),digits=2)
# Both tests indicate strong positive relations between the variables.
...
# For the Girls' and Boys' datasets combined:
ggpairs(
  data.set,
  upper = list(wrap("cor", size = 5)),
  diag = list(continuous = "densityDiag"),
